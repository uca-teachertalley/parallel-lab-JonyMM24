...
// This function walks through an array and counts the prime numbers in parallel
...
int main() {
	...
	for (int i = 0; i < 10; i++) {
		...
		count = count_prime_parallel(numbers, n);
		end = chrono::steady_clock::now();
		// Print results
		compute_time = chrono::duration<double>(end - start).count();
		compute_list[i] = compute_time;

	}
	double avg_time = 0;
	for (int x = 0; x < 10; x++) {
		avg_time += compute_list[x];
	}
	//cout << "Total number of primes = " << count << endl;
	cout << "Average computation time = " << avg_time / 10 << endl;
    return 0;
}
